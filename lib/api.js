...
class TestingBot {
  ...
  request (reqData, callback) {
    const executeRequest = (resolve, reject) => {
      let requestPath = '/v1' + reqData.url;
      if (reqData.method === 'GET' && reqData.data) {
        requestPath = requestPath + '?' + querystring.stringify(reqData.data);
      }
      const requestOptions = {
      ...
      if (reqData.method !== 'GET') {
        if (reqData.json) {
          ...
          requestOptions.body = reqData.data;
        } else {
          requestOptions.form = reqData.data;
        }
      }
      ...
      request(requestOptions, function (error, response) {
        let responseBody = null;
        if (response) {
          if (!reqData.json && response.body && typeof (response.body) === 'string') {
            try {
              response.body = JSON.parse(response.body);
            } catch (e) {
